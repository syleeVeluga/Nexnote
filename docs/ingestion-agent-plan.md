...
> **상태**: 승인됨 (2026-04-29) · AGENT-1~7 + AGENT-4.5 완료, AGENT-8 시작 (2026-04-30) · pre-promotion hardening 3개 해소, production 'agent' 승격은 parity 관측/단계 전환 후
> **유형**: 구현 RFC (PRD 아님)
> **모티브**: [Karpathy gist — LLM Wiki](https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f) 의 _"한 번의 ingest로 LLM이 10–15개 페이지를 동시에 갱신한다"_ 패턴
...
- [route-classifier.ts:240](../packages/worker/src/workers/route-classifier.ts#L240) ROUTE_SYSTEM_PROMPT가 _"Be conservative — prefer needs_review if confidence < 0.6"_ 로 보수 편향
...
목표 결과: Karpathy gist의 _"한 번의 ingest로 LLM이 10–15개 페이지를 동시에 갱신한다"_ 패턴을 도입. 한 ingestion이 search/read 도구를 능동적으로 써서 워크스페이스를 탐색한 뒤, 필요한 만큼의 create/update/append 결정을 fan-out으로 발행한다. 모든 안전장치(0.85/0.60 confidence 게이트, baseRevisionId 충돌 검출, audit_logs, model_runs)는 그대로 보존.
...
순수 ReAct는 update_page 발행 후 후속 관측이 다른 페이지가 맞다고 알려주는 _commit-too-early_ 사고가 잦다. 따라서:
...
**Plan turn에서 tier 선택을 강제**: agent가 mutation을 제안할 때 가장 좁은 tier를 우선. Plan validator가 _"전체 페이지 70% 이상이 동일하면 update_page 금지, edit_page_blocks로 분해하라"_ 규칙으로 self-correct 1턴 부여. Tier 1·2·3는 **patch-generator 호출 없이** 직접 새 revision을 만든다 (LLM 재호출 없음 → 비용 절감 + 의도 보존). update_page/append_to_page만 patch-generator를 거침.
...
| 신규 `AISettingsPage.tsx` + 사이드바 nav                                     | `/settings/ai` 모드 토글 (classic/shadow/agent), 모델·budget 편집           |
...
- `packages/worker/src/lib/agent/tools/mutate.ts` — 8종 mutate (replace_in_page, edit_page_blocks, edit_page_section, update_page, append_to_page, create_page, noop, request_human_review), decision/revision wrapper
- `packages/worker/src/lib/agent/patch/inline-patch.ts` — replace_in_page (find/replace 검증, 정확히 N매치 enforce)
- `packages/worker/src/lib/agent/patch/block-patch.ts` — 기존 stable block parser 기반 blockId 매칭, blockOps 적용
...
- **Gate active (2026-04-30)** — `/settings/ai` can start shadow observation, `/workspaces/:id/agent-runs/diagnostics` returns daily rows + gate status, and `PATCH /workspaces/:id` refuses `agent` promotion until the gate passes. Defaults: 7 observed days, 20 comparable ingestions, action agreement ≥0.90, target-page agreement ≥0.85; override with `AGENT_PARITY_GATE_MIN_*`.

  4.5. **AGENT-4.5 · [DONE · 2026-04-30] Shadow hardening before parity gate** — Phase B/C bridge 작업으로, parity SQL view + `agent_vs_classic_agreement_rate` 뷰, `workspaces.agent_instructions` 컬럼 + system prompt prepend, workspace daily token cap end-to-end enforce, dedupe system-message hint, Redis pub/sub 기반 SSE live trace, oldest-first 컨텍스트 압축(80% 임계 + cache 무효화) 모두 머지. AGENT-5와 병렬로 진행되어 production 진입 전 운영 가시성/안전망을 일괄 확보. **차단**: AGENT-4. **sub-doc 없음** (TASKS.md 항목으로 추적).

### Phase C — Go-live (Parity gate 통과 후, 5/6/7 병렬 가능)

5. **AGENT-5 · [DONE · 2026-04-29] Mutate tool wrappers (3-tier patches)** — `replace_in_page` / `edit_page_blocks` / `edit_page_section`는 LLM 재호출 없이 직접 revision 생성, `update_page` / `append_to_page`는 patch-generator fallback. 완료: sub-doc `step-5-mutate-tiers.md`, mutate schemas/tools, 직접 revision + provenance/diff/audit enqueue, 동일 페이지 락, seen page/block 검사, `agent` 모드 decision fan-out ownership, 80% oldest-first context compaction + re-fetch notice, mutate error self-correction hints + 1회 repair turn. 남음: 70% full-rewrite self-correct.
6. **AGENT-6 · [DONE · 2026-04-30] Workspace 토글 + `/settings/ai`** — 기본 'classic', 사내 워크스페이스부터 'shadow' → 'agent'. 완료: workspace-scoped `agent_provider`, fast/large-context model picker, fast-threshold, daily token cap, `/settings/ai` route + `/system/ai` redirect, diagnostics effective-settings 노출. sub-doc 없음
7. **AGENT-7 · [DONE · 2026-04-30] UI fan-out** — IngestionDetailPage decision[] 렌더 ✅, AgentTracePanel (post-hoc + Redis/SSE live) ✅, ReviewQueuePage sibling 배지 "(N of M from ingestion X)" ✅, Activity feed `agent_run_completed` 행 ✅. **차단**: AGENT-2 (`agent_run_id` FK). 실효성은 AGENT-4 이후. sub-doc 없음

### Phase D — Cleanup

8. **AGENT-8 · [IN PROGRESS · 2026-04-30] Cutover & retire classic** — 시작 작업 완료: `read_page` large-markdown blocks fallback, agent-mode execute smoke, model diagnostic strip, BullMQ-safe agent job IDs. 다음은 워크스페이스 단위 단계 전환, 토큰 비용 + parity 대시보드 모니터링, **'agent' 모드 2주 클린 운영** 후 classic route-classifier 제거. 기존 classic decision 행은 NULL FK로 보존. sub-doc 없음
...
**유닛/통합** (체크: 2026-04-30 코드 실측 기준)

- [x] AI gateway conformance test: 동일 fixture (system+user+tools) 입력 시 OpenAI vs Gemini 어댑터가 동일 normalized `toolCalls[]` 반환
- [x] Tool dispatcher 위조 workspaceId 인자 무시 테스트 (cross-workspace leak)
- [x] UUID hallucination 방어: 본 run에서 관측 안 된 pageId로 update_page 호출 시 recoverable error
- [x] 동일 turn 내 동일 pageId 병렬 update 거부
- [x] Per-tool quota 초과 시 dispatcher가 거부, agent loop는 종료 가능
- [x] approveDecision 호출 시 audit_logs / model_runs / baseRevisionId 충돌 다운그레이드가 classic과 동일하게 동작
- [x] **replace_in_page**: find가 0매치/N>1매치/공백차이 시 명확한 recoverable error, 정확 1매치 시 lineDiff에 정확히 1줄 변경만 기록
- [x] **edit_page_blocks**: 본 run에서 관측 안 된 blockId 거부, replace/insert_after/insert_before/delete 4종 op 정상 동작, 결과 markdown round-trip (parse → ops → render → parse 동일)
- [x] **edit_page_section**: heading anchor 정확 1매치 enforce, 섹션 경계가 다음 동일/상위 레벨 heading까지 정확히 인식
- [x] **budgeter**: 누적 메시지가 모델 한계 초과 직전 가장 오래된 read를 summary로 압축, agent에 통지 system message 삽입, 후속 read_page 재호출이 캐시 히트 (mutate 후엔 무효화)
- [x] **모델 라우팅**: 추정 입력 < 50k면 fast 모델, 이상이면 large-context 모델 선택 — `agent_runs.steps_json[0]`에 결정 사실 기록
- [x] **Dedupe system-message hint**: 동일 args read 호출 캐시 히트 시 `loop.ts` 가 후속 system message 주입 (RFC §Tool dedupe)
- [ ] **Plan validator self-correct**: 70% 이상 동일한 update_page 제안을 edit_page_blocks로 분해 재요청 — 잔여 (AGENT-5 hardening)
- [x] **Mutate execute 통합 테스트**: agent 모드에서 mutate tool → ingestion_decisions + page_revisions + audit_logs + agentRunId FK가 끝까지 채워지는 happy/conflict 경로 검증. `pipeline.smoke.test.ts`가 agent-mode `create_page` happy path와 direct patch human-conflict downgrade를 커버하고, repair turn은 기존 `loop.test.ts` self-correction 케이스가 커버.
- [x] **read_page 큰 본문 자동 blocks 폴백**: 본문이 모델 입력 capacity 일정 비율 또는 30k-token 상한을 초과 시 `format=markdown` 요청을 compact `blocks` 결과로 자동 전환하고 LLM system notice 주입.
...
- [x] 같은 워크스페이스를 `ingestion_mode='agent'`로 승격 → ingestion_decisions에 agent-owned row가 생성되고 revision/audit/triple까지 연결되는 smoke 검증 (`pipeline.smoke.test.ts`; fan-out PDF E2E는 운영 시나리오로 유지)
- [x] AgentTracePanel이 thought / tool_call / tool_result 순서대로 표시 (post-hoc + Redis pub/sub SSE live)
- [ ] 모드를 'classic'으로 되돌렸을 때 즉시 classic 경로로 복귀 (재배포 불필요) — enqueue 분기는 구현됨, smoke 잔여

**모니터링**

- [x] daily `agent_vs_classic_agreement_rate` SQL view + `/agent-runs/diagnostics` API + AISettingsPage 대시보드 (1주 누적은 운영 사항)
- [ ] `agent_runs` 토큰 P50/P95/P99 — P50이 30k 초과 시 알림 (수동 SQL은 가능, alerting 미연결)
- [x] Workspace daily token cap end-to-end enforce — `loop.ts` reservation + `releaseWorkspaceTokenReservation`

---

## Remaining gaps before 'agent' promotion (2026-04-30 실측)

이전 점검에서 식별한 5갭 중 네 개 (dedupe system-message hint, oldest-first 컨텍스트 압축, ReviewQueuePage sibling 배지, Activity feed agent_run 행) 는 이미 코드에 들어가 있었다. AGENT-8 시작 PR에서 남은 pre-promotion hardening 3개도 코드에 들어갔다:

| 우선순위                 | 갭                                        | 영향                                                                              | 위치                                                                                                                          | 사이즈 |
| ------------------------ | ----------------------------------------- | --------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- | ------ |
| **DONE · AGENT-8 start** | 1. `read_page` 큰 본문 자동 `blocks` 폴백 | 단일 거대 페이지(>30k 토큰)가 한 턴 입력 한도를 잠식 → context overflow / timeout | [tools/read.ts](../packages/worker/src/lib/agent/tools/read.ts) + [budgeter.ts](../packages/worker/src/lib/agent/budgeter.ts) | S      |
| **DONE · AGENT-8 start** | 2. mutate execute 통합 테스트             | shadow 외 실제 쓰기 경로 end-to-end 회귀 안전망 부재 → production 사고 위험       | [pipeline.smoke.test.ts](../tests/integration/pipeline.smoke.test.ts) + 기존 `loop.test.ts` repair coverage                   | M      |
| **DONE · AGENT-8 start** | 3. AISettingsPage 모델 진단 strip         | 운영자가 "현재 어떤 모델이 fast/large_context로 라우트되는지" 화면에서 확인 불가  | `/diagnostics` `currentModels` + [AISettingsPage.tsx](../packages/web/src/pages/AISettingsPage.tsx)                           | S      |

이제 사내 워크스페이스부터 `shadow` parity 임계 통과 → `agent` 승격을 진행할 수 있다. 글로벌 classic retire는 아래 out-of-scope 항목처럼 `agent` 모드 2주 클린 운영 후 별도 cleanup PR로 한다.
...
- 3D 그래프 토글
- **AGENT-8 글로벌 classic retire** — 'agent' 모드 2주 클린 운영 후 별도 cleanup PR. 현재 워크스페이스 단위 전환은 enqueue-ingestion이 이미 분기하므로 v1 production 진입에 무관
