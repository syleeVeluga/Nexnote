...
> **상태**: 승인됨 (2026-04-29) · AGENT-1~7 + AGENT-4.5 완료 (~2026-04-30), AGENT-8 Phase D 보류 · production 'agent' 승격은 아래 §Remaining gaps 3개 해소 후
> **유형**: 구현 RFC (PRD 아님)
...
이전 점검에서 식별한 5갭 중 네 개 (dedupe system-message hint, oldest-first 컨텍스트 압축, ReviewQueuePage sibling 배지, Activity feed agent_run 행) 는 이미 코드에 들어가 있다. 잔여는 3개:
...
| **P2 — 마감재** | 3. AISettingsPage 모델 진단 strip | 운영자가 "현재 어떤 모델이 fast/large_context로 라우트되는지" 화면에서 확인 불가 | env-only 진단 strip. `/diagnostics` 응답에 `currentModels` 추가, AISettingsPage에 read-only 표시 | S |

권장 PR 묶음:
- **PR-A**: P0 두 개 (read fallback + execute test). 두 작업 모두 worker test infra를 공유 — bundling 자연스러움
- **PR-B**: P2 (모델 진단 strip). 단독 30줄 수준

P0 두 개 머지 + parity gate 임계 통과 후 사내 워크스페이스부터 'agent' 승격 가능. P2는 후속 PR로 무방.
