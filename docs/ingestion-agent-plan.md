...
- [enqueue-ingestion.ts](../packages/api/src/lib/enqueue-ingestion.ts)이 모드를 읽어 `JOB_NAMES.ROUTE_CLASSIFIER` vs `JOB_NAMES.INGESTION_AGENT` 선택 (큐는 동일 `QUEUE_NAMES.INGESTION`, 두 워커 동시 등록 — BullMQ 네이티브 패턴)
...
1. **AI gateway 확장** + OpenAI/Gemini conformance test. 다른 모든 작업의 prerequisite
2. **`0015_agent_runs.sql` migration + Drizzle schema** + workspaces.ingestion_mode (NULL FK라 backwards-compat)
3. **Read-only tool layer + dispatcher** (workspaceId closed-over, quotas, dedupe). 단독 실행 가능한 모듈로 격리 검증
4. **Agent loop in shadow mode** — `agent_runs.plan_json`만 기록, ingestion_decisions는 안 건드림. 1주일 합의도 관찰
5. **Mutate tool wrappers** — apply-decision/patch-generator delegate. 동일 페이지 락, seenUUIDs 검사
6. **Workspace 토글 + AISettingsPage** — 기본 'classic', 사내 워크스페이스부터 'shadow' → 'agent'
7. **UI fan-out** — IngestionDetailPage decision[] 렌더, ReviewQueuePage 형제 배지, AgentTracePanel
8. **Promote 워크스페이스 단위로 단계 전환**, 토큰 비용 + parity 대시보드 모니터링, 2주 클린 후 classic 제거

가장 leveraged 결정: 1번 gateway 정규화. 가장 위험한 누락: 4번 shadow mode. 그 외는 회복 가능.
