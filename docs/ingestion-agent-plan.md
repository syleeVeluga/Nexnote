...
> **상태**: 승인됨 (2026-04-29) · AGENT-1~6 + AGENT-4.5 완료 (~2026-04-30), AGENT-7 부분 완료 (sibling 배지 / activity row 잔여), AGENT-8 Phase D 보류 · production 'agent' 승격은 아래 §Remaining gaps 5개 해소 후
> **유형**: 구현 RFC (PRD 아님)
...
7. **AGENT-7 · [PARTIAL · 2026-04-30] UI fan-out** — IngestionDetailPage decision[] 렌더 ✅, AgentTracePanel (post-hoc + Redis/SSE live) ✅. 잔여: ReviewQueuePage sibling 배지 "(N of M from ingestion X)" + Activity feed에 agent_run 완료 행. **차단**: AGENT-2 (`agent_run_id` FK). 실효성은 AGENT-4 이후. sub-doc 없음
...
- [ ] **Mutate execute 통합 테스트**: agent 모드에서 mutate tool → ingestion_decisions + page_revisions + audit_logs + agentRunId FK가 끝까지 채워지는 happy/conflict/repair 시나리오 — 잔여 (P0)
- [ ] **read_page 큰 본문 자동 blocks 폴백**: 본문이 모델 입력 capacity 일정 비율 초과 시 dispatcher 레벨에서 `format=markdown` → `blocks` 강제 + LLM 통지 — 잔여 (P0)
...
이전 점검에서 식별한 5갭 중 두 개 (dedupe system-message hint, oldest-first 컨텍스트 압축) 는 이미 코드에 들어가 있다. 잔여는 5개:

| 우선순위 | 갭 | 영향 | 위치 | 사이즈 |
|---|---|---|---|---|
| **P0 — 'agent' 승격 차단** | 1. `read_page` 큰 본문 자동 `blocks` 폴백 | 단일 거대 페이지(>30k 토큰)가 한 턴 입력 한도를 잠식 → context overflow / timeout | [tools/read.ts:415-489](../packages/worker/src/lib/agent/tools/read.ts) 의 `readPage` + 신규 helper in `budgeter.ts` | S |
| **P0 — 'agent' 승격 차단** | 2. mutate execute 통합 테스트 | shadow 외 실제 쓰기 경로 end-to-end 회귀 안전망 부재 → production 사고 위험 | 신규 `packages/worker/src/lib/agent/loop.execute.test.ts` + 기존 `SequenceAdapter` / `createMutateTools` 재사용 | L |
| **P1 — 운영 품질** | 3. ReviewQueuePage sibling 배지 | fan-out된 N개 decision이 같은 ingestion에서 왔다는 시각 단서 부재 → 검토 시 맥락 단절 | [ReviewQueuePage.tsx:351-409](../packages/web/src/pages/ReviewQueuePage.tsx) — backend 변경 불필요(`decisions.list`가 이미 `ingestion: { id, sourceName }` 반환) | S |
| **P1 — 운영 품질** | 4. Activity feed 에 agent_run 행 | "오늘 agent가 뭘 했지?" 답변이 audit_logs에서 derive 됐을 때 한 줄로 안 보임 | `audit_logs` row 1개 추가 (action='agent_run_completed') + `deriveActivitySummary` 확장. 스키마 변경 없음 | M |
| **P2 — 마감재** | 5. AISettingsPage 모델 진단 strip | 운영자가 "현재 어떤 모델이 fast/large_context로 라우트되는지" 화면에서 확인 불가 | env-only 진단 strip. `/diagnostics` 응답에 `currentModels` 추가, AISettingsPage에 read-only 표시 | S |

권장 PR 묶음:
- **PR-A**: P0 두 개 (read fallback + execute test). 두 작업 모두 worker test infra를 공유 — bundling 자연스러움
- **PR-B**: P1 두 개 (sibling 배지 + activity row). UI/관측성 한 묶음
- **PR-C**: P2 (모델 진단 strip). 단독 30줄 수준

P0 두 개 머지 + parity gate 임계 통과 후 사내 워크스페이스부터 'agent' 승격 가능. P1·P2는 후속 PR로 무방.
