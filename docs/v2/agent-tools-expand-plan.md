...
> **상태**: 구현 완료 검증 (2026-05-05) · S2·S3 landed
> **유형**: 구현 RFC
...
S2/S3 는 현재 코드베이스에 반영돼 있고, 관련 worker/api/web 검증도 통과했다.

- S2: `move_page`, `rename_page`, `create_folder` mutate tool, dispatcher folder observation, trace/activity labels, reorganize tests landed.
- S3: `read_page_metadata`, `find_backlinks`, `read_revision_history`, `read_revision`, frontmatter parser, read quotas, prompt guidance, read-intel tests landed.
- 검증 기준선: `pnpm --filter @wekiflow/worker test`, `pnpm --filter @wekiflow/api test`, `pnpm --filter @wekiflow/web test`, `pnpm typecheck`, `pnpm --filter @wekiflow/web build`.
...
- [`docs/TASKS.md`](../TASKS.md) 에 AUTO-2 (S2) / AUTO-3 (S3) landed 상태를 반영.
- [`CLAUDE.md`](../../CLAUDE.md) 는 현재 `AGENTS.md` 위임 엔트리만 유지하므로 별도 상태 표 갱신 없음.
