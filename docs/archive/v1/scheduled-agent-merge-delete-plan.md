...
> Update 2026-05-02: destructive Scheduled Agent tools now auto-apply and permanently purge affected source subtrees. Earlier "always suggested / review first" notes in this historical RFC are superseded by the autonomous policy in `docs/scheduled-agent-plan.md`.
...
- **자동 반영** — destructive Scheduled Agent 도구는 review 승인 없이 적용하고, source subtree는 휴지통에 남기지 않고 purge
...
delete_page / merge_pages   ← NEW mutate tools (scheduled origin only)
   ...
   action='delete'  status='auto_applied'  rationaleJson={ pageIds: [...] }
   action='merge'   status='auto_applied'  rationaleJson={ canonicalPageId, sourcePageIds[] }
...
- **`scheduledPromptPrefix`** (line 253-271): 한 줄 추가 —
  ...
  - Use merge_pages to consolidate 2+ short pages into a canonical page; provide the
    full mergedContentMd. delete_page and merge_pages auto-apply in scheduled-origin
    runs and permanently purge affected source subtrees.
  ```
...
3. **`createDecision()`** 호출:
   - `action: "delete"`, `status: "auto_applied"`, `targetPageId: args.pageId`,
...
2. canonical에 대해 `detectHumanConflict`, source 각각도 동일하게 검사 — 충돌 시 `rationaleJson.conflict`에 기록하고 자동 반영한다.
...
4. **`createDecision()`** 호출:
   - `action: "merge"`, `status: "auto_applied"`, `targetPageId: canonicalPageId`,
...
| `confidence < 0.60`                    | scheduled origin에서는 그대로 자동 반영하거나 `noop` 처리 |
| 어떤 confidence든                      | `status='auto_applied'`                                   |
| canonical 또는 source에 사람 편집 충돌 | `rationaleJson.conflicts`에 기록하고 자동 반영            |
| sourcePageIds.length > 10              | Zod에서 거부                                              |
| 자식 페이지가 있는 page를 delete       | subtree 전체를 purge                                      |
...
1. **단위 테스트** (`packages/worker/src/lib/agent/tools/mutate.test.ts` 또는 신규):
   ...
   - 두 도구 모두 `status='auto_applied'`로 작성
...
2. **apply-decision 단위 테스트** (`packages/api/src/lib/apply-decision.test.ts` 추가):
   - delete 실행 → source subtree purge, decision `status='auto_applied'`, audit log 기록
   - merge 실행 → canonical promoted + sources purged + triple/search 큐 enqueue
3. **e2e 수동 시나리오**:
   ...
   - `/review`에는 승인 카드가 생기지 않음.
   - 실행 완료 → 4개 중 3개 source page가 purge되고 canonical 페이지가 통합 본문으로 갱신.
