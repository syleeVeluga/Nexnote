...
## Current Implementation Status (snapshot: 2026-05-01, docs reviewed)
...
| ② **Classify** | ✅ DONE (classic) · ✅ DONE (agent backend) · 🟡 PROMOTE-GATED | Classic [route-classifier.ts](packages/worker/src/workers/route-classifier.ts) still owns classic/shadow decisions; in `ingestion_mode='agent'` the new agent backend owns decisions. **Agent backend** ([`docs/ingestion-agent-plan.md`](docs/ingestion-agent-plan.md)): AGENT-1/2/3/4/4.5/5/6/7 and AGENT-8 start landed by 2026-04-30 — normalized tool-calling gateway (OpenAI/Gemini), `agent_runs` schema + `workspaces.ingestion_mode` + `workspaces.agent_instructions`, read-only dispatcher (workspaceId closure, quota, dedupe, seenPageIds/seenBlockIds), shadow trace loop with budgeter + model routing, **parity SQL view + `/agent-runs/diagnostics` API**, daily token cap enforcement, dedupe system-message hint, **Redis pub/sub SSE live trace + `AgentTracePanel`**, mutate tier 1·2·3 (`replace_in_page` exact-N match / `edit_page_blocks` seenBlockIds / `edit_page_section` heading anchor) building revisions directly without re-LLM, `update_page`/`append_to_page` patch-generator fallback, oldest-first 80% context compaction with cache invalidation, mutate self-correction repair turn, `/settings/ai` workspace toggle + workspace-scoped model picker + parity dashboard + model diagnostic strip, fan-out review/activity surfaces, large `read_page` auto blocks fallback, BullMQ-safe agent job IDs, and agent-mode execute smoke coverage. **Promotion to `agent` is server-gated** by `PATCH /workspaces/:id` — refuses until 7 observed days × 20 comparable ingestions × ≥0.90 action / ≥0.85 target-page agreement (env-tunable via `AGENT_PARITY_GATE_MIN_*`). Broad rollout now depends on parity observation / staged workspace promotion; global classic retirement waits for 2 weeks of clean `agent` operation. |
...
The ingestion-agent epic (RFC: [`docs/ingestion-agent-plan.md`](docs/ingestion-agent-plan.md)) closed its backend and operator surfaces by 2026-04-30: tool-calling agent runs in shadow alongside the classic classifier, can fan out one ingestion into multiple surgical decisions across existing pages, and a server-side parity gate refuses promotion to `agent` mode until action / target-page agreement thresholds are met. `/settings/ai` exposes the toggle, workspace-scoped model picker, daily token cap, and parity dashboard; `AgentTracePanel` streams the explore→plan→execute trace live via SSE. Scheduled Agent (RFC/status: [`docs/scheduled-agent-plan.md`](docs/scheduled-agent-plan.md)) now reuses that same loop for manual and cron-driven wiki maintenance with settings UI, task/run UI, folder trigger, and trace viewing.

What remains on the path to "AI keeps the wiki continuously up-to-date under human supervision": Scheduled Agent review-origin/e2e polish; parity observation and staged `agent` workspace promotion, then 2 weeks of clean `agent` operation before global classic retirement; concurrent-ingestion detection and triple-level contradictions; API-token management; sidebar badges/digests; AI-edit accept/reject UX; workspace-wide graph exploration; broader CI/integration coverage.

**Next major direction:** finish Scheduled Agent supervision polish, then begin gradual workspace promotion under the existing parity gate, monitor token cost/parity, and defer global classic retirement to the cleanup PR after 2 weeks of clean `agent`-mode operation.
