...
| ② **Classify** | ✅ DONE (classic) · ✅ DONE (agent backend) · 🟡 PROMOTE-GATED | Classic [route-classifier.ts](packages/worker/src/workers/route-classifier.ts) still owns classic/shadow decisions; in `ingestion_mode='agent'` the new agent backend owns decisions. **Agent backend** ([`docs/ingestion-agent-plan.md`](docs/ingestion-agent-plan.md)): AGENT-1/2/3/4/4.5/5/6 landed by 2026-04-30 — normalized tool-calling gateway (OpenAI/Gemini), `agent_runs` schema + `workspaces.ingestion_mode` + `workspaces.agent_instructions`, read-only dispatcher (workspaceId closure, quota, dedupe, seenPageIds/seenBlockIds), shadow trace loop with budgeter + model routing, **parity SQL view + `/agent-runs/diagnostics` API**, daily token cap enforcement, dedupe system-message hint, **Redis pub/sub SSE live trace + `AgentTracePanel`**, mutate tier 1·2·3 (`replace_in_page` exact-N match / `edit_page_blocks` seenBlockIds / `edit_page_section` heading anchor) building revisions directly without re-LLM, `update_page`/`append_to_page` patch-generator fallback, oldest-first 80% context compaction with cache invalidation, mutate self-correction repair turn, and `/settings/ai` workspace toggle + workspace-scoped model picker + parity dashboard. **Promotion to `agent` is server-gated** by `PATCH /workspaces/:id` — refuses until 7 observed days × 20 comparable ingestions × ≥0.90 action / ≥0.85 target-page agreement (env-tunable via `AGENT_PARITY_GATE_MIN_*`). Still missing before broad rollout (RFC §Remaining gaps): `read_page` auto blocks fallback (P0), mutate execute integration test (P0), ReviewQueuePage sibling badge (P1), Activity feed agent_run row (P1), AISettingsPage model diagnostic strip (P2). |
...
The ingestion-agent epic (RFC: [`docs/ingestion-agent-plan.md`](docs/ingestion-agent-plan.md)) closed its backend and operator surfaces by 2026-04-30: tool-calling agent runs in shadow alongside the classic classifier, can fan out one ingestion into multiple surgical decisions across existing pages, and a server-side parity gate refuses promotion to `agent` mode until action / target-page agreement thresholds are met. `/settings/ai` exposes the toggle, workspace-scoped model picker, daily token cap, and parity dashboard; `AgentTracePanel` streams the explore→plan→execute trace live via SSE.

What remains on the path to "AI keeps the wiki continuously up-to-date under human supervision": (a) **production 'agent' rollout finishing gaps** — `read_page` auto blocks fallback (P0, prevents single oversized page from blowing context), mutate execute integration test (P0, end-to-end write-path safety net), ReviewQueuePage sibling badge (P1), Activity feed agent_run row (P1), AISettingsPage model diagnostic strip (P2); (b) concurrent-ingestion detection and triple-level contradictions; (c) API-token management; (d) sidebar badges/digests; (e) AI-edit accept/reject UX; (f) workspace-wide graph exploration; (g) broader CI/integration coverage.

**Next major direction:** ship the 5 'agent' promotion finishing gaps in PR-A (P0) → PR-B (P1) → PR-C (P2) order, then begin gradual workspace promotion under the existing parity gate. AGENT-8 (global classic retire) remains deferred to v2 cleanup after 2 weeks of clean `agent`-mode operation.
