...
| ② **Classify** | ✅ DONE (classic) · 🟡 FOUNDATION (agent) | Current: [route-classifier.ts](packages/worker/src/workers/route-classifier.ts) (523L) does title + FTS + trigram + entity-overlap candidate search, LLM picks action + confidence. Writes `ingestion_decisions`. **Agent replacement foundation** ([`docs/ingestion-agent-plan.md`](docs/ingestion-agent-plan.md)): AGENT-1/2/3 landed — normalized tool-calling gateway, `agent_runs` schema + `workspaces.ingestion_mode`, and read-only `search_pages` / `read_page` / `list_folder` / `find_related_entities` / `list_recent_pages` dispatcher with quota/dedupe/seen-id tracking. Still missing: AGENT-4 shadow loop, parity gate, mutate tools, settings UI, and fan-out review UI. |
