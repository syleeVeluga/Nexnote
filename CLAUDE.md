...
| ② **Classify** | ✅ DONE (classic) · 🟡 AGENT BACKEND | Current: [route-classifier.ts](packages/worker/src/workers/route-classifier.ts) still owns classic/shadow decisions. **Agent replacement backend** ([`docs/ingestion-agent-plan.md`](docs/ingestion-agent-plan.md)): AGENT-1/2/3/4/5 landed — normalized tool-calling gateway, `agent_runs` schema + `workspaces.ingestion_mode`, read-only dispatcher, shadow trace loop, typed mutate tools, direct patch tiers, agent-mode decision fan-out with `agent_run_id`, oldest-first context compaction, and mutate error repair hints. Still missing before broad rollout: parity observation, settings UI, fan-out review UI, 70% full-rewrite self-correct, and broader DB integration coverage. |
...
**Next major direction:** finish rollout surfaces for the tool-calling ingestion agent — settings toggle, fan-out review UI, trace panel, and parity/coverage hardening — before retiring classic classifier ownership. See [`docs/ingestion-agent-plan.md`](docs/ingestion-agent-plan.md).
