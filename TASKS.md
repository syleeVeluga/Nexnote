...
> **Tranche 1 landed (2026-04-17):** migration `0003_supervision_loop_foundations` (search_vector column + GIN index, `page_revisions.source_ingestion_id` + `source_decision_id` FKs, `ingestion_decisions.status`); route-classifier now does three-band routing and tags decisions `auto_applied` / `suggested` / `needs_review` / `noop`; patch-generator populates provenance FKs and sets status on success/failure; the `POST /ingestions/:id/apply` endpoint transitions decision status to `approved` / `rejected` on human action. Next: S3-2 (richer approve/reject API) and S4-1 (review queue UI).

---

## Stage ③ — Remaining follow-ups

### S3-2 · [HIGH] Approve / reject / edit API for pending decisions
Current `POST /ingestions/:id/apply` takes a single `approved` flag. The review UI needs richer ops.
- `POST /workspaces/:id/decisions/:decisionId/approve` — runs patch-generator on the decision
- `POST /.../decisions/:decisionId/reject` — marks `status="rejected"`, stores reviewer + reason
- `PATCH /.../decisions/:decisionId` — edit targetPageId, editType, or action before approving (e.g. "this should create a new page, not update the existing one")
- All write `audit_logs` with reviewer identity
...
## Stage ④ — Human review UI (MISSING — biggest gap toward the goal)

There is currently **zero frontend surface** for pending AI decisions. The whole supervision story depends on this existing.

### S4-1 · [HIGH] `/workspaces/:slug/review` — unified review queue
- Tabs: **Suggested** (0.60–0.84), **Needs review** (<0.60), **Failed** (worker errors), **Recent** (last 7d auto-applied)
- Each row: source name, target page (or "new page: …"), action, confidence, AI reason excerpt, time
- Click → detail panel with: raw ingestion text, proposed diff (for update/append), Approve / Reject / Edit buttons
- Keyboard shortcuts: `a` approve, `r` reject, `j/k` nav — throughput matters for a supervision role
- Counts in the workspace sidebar so reviewers know when queue has items
...
### S4-3 · [MED] API-client surface for ingestions + decisions
[api-client.ts](packages/web/src/lib/api-client.ts) currently has no ingestion methods (only workspaces/folders/pages/auth). Add: `listIngestions`, `getIngestion`, `listDecisions`, `approveDecision`, `rejectDecision`, `editDecision`.
