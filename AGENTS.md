...
What remains on the path to "AI keeps the wiki continuously up-to-date under human supervision": parity observation and staged `agent` workspace promotion, 2 weeks of clean `agent` operation before global classic retirement, concurrent-ingestion detection, triple-level contradictions, API-token management, sidebar badges/digests, AI-edit accept/reject UX, workspace-wide graph exploration, and broader CI/integration coverage.

**Next major direction:** begin gradual workspace promotion under the existing parity gate, monitor token cost/parity, and defer global classic retirement to the cleanup PR after 2 weeks of clean `agent`-mode operation. See [`docs/ingestion-agent-plan.md`](docs/ingestion-agent-plan.md).
