...
| — **Graph exploration** | 🟡 PARTIAL | Per-page BFS endpoint + graph side panel + type-colored nodes + predicate edge labels ✅. Editor-side ontology panel now includes entity-type / predicate filters, confidence slider, 2D/3D toggle, focus dimming, edge width/opacity encoding, and a node inspector with direct incoming/outgoing relations plus provenance/source pages. **Missing:** workspace-wide graph, aliases/full triple browser + entity actions, node search/pathfinding, persisted graph preferences, export/time-lapse tooling. |
